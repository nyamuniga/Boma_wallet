...
pub fn export_watch_wallet(root_key: &ExtendedPrivKey, network: Network) -> Result<(), String> {
    let xpub = get_account_xpub(root_key, network)?;
    ...
    let mut f = std::fs::File::create("watch_wallet.txt")
        .map_err(|e| format!("Failed to create watch_wallet.txt: {}", e))?;
    f.write_all(contents.as_bytes())
        .map_err(|e| format!("Failed to write watch_wallet.txt: {}", e))?;

    Ok(())
}

/// Writes the output descriptor to wallet_descriptor.txt.
pub fn export_descriptor(root_key: &ExtendedPrivKey, network: Network) -> Result<(), String> {
    let descriptor = get_descriptor(root_key, network)?;
    let mut f = std::fs::File::create("wallet_descriptor.txt")
        .map_err(|e| format!("Failed to create wallet_descriptor.txt: {}", e))?;
    f.write_all(format!("{}\n", descriptor).as_bytes())

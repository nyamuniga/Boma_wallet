...
/// Generates the watch-only wallet file content as a string.
/// This includes the xpub, descriptor, fingerprint, and informational comments.
pub fn watch_wallet_content(root_key: &ExtendedPrivKey, network: Network) -> Result<String, String> {
    let xpub = get_account_xpub(root_key, network)?;
...
pub fn export_watch_wallet(root_key: &ExtendedPrivKey, network: Network, save_path: &str) -> Result<(), String> {
    let contents = watch_wallet_content(root_key, network)?;
...
/// Generates the output descriptor string for the wallet's external receive chain.
/// Format: wpkh(xpub/0/*) — compatible with Electrum, Sparrow, Bitcoin Core.
pub fn descriptor_content(root_key: &ExtendedPrivKey, network: Network) -> Result<String, String> {
    let descriptor = get_descriptor(root_key, network)?;
    Ok(format!("{}\n", descriptor))
}
...
pub fn export_descriptor(root_key: &ExtendedPrivKey, network: Network, save_path: &str) -> Result<(), String> {
    let contents = descriptor_content(root_key, network)?;
    let mut f = std::fs::File::create(save_path)
        .map_err(|e| format!("Failed to create {}: {}", save_path, e))?;
    f.write_all(contents.as_bytes())
